...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-charcoal/10 bg-warmwhite/95 backdrop-blur-md">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
        <Link href="/" className="flex items-center gap-2 sm:gap-3">
          <Image
            ...
            priority
          />
          <span className="font-serif text-base font-semibold text-charcoal sm:text-xl">
            Migration Pathway
          </span>
        </Link>
      ...
      {/* Mobile Dropdown */}
      {menuOpen && (
        <div className="border-t border-charcoal/10 bg-warmwhite px-4 pb-4 pt-2 sm:px-6 lg:hidden">
          <div className="flex flex-col gap-1">
            {navLinks.map((link) => (
              <Link
                ...
                className="rounded-lg px-3 py-2.5 text-sm font-medium text-slate transition hover:bg-charcoal/5 hover:text-charcoal"
              >
                {link.label}
              </Link>
            ))}
